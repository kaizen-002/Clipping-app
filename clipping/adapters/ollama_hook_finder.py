...
from clipping.core.models import HookCandidate, Transcript
...
_SYSTEM_PROMPT = """You find the single most engaging 15-60 second segment of a \
podcast transcript — the part that would stop someone scrolling.
...
{"start_seconds": <number>, "end_seconds": <number>, "reason": "<one sentence>"}

Both timestamps must be seconds that appear in the transcript below."""
...
class OllamaHookFinder:
    ...
    def find(self, transcript: Transcript) -> HookCandidate:
        ...
        return self._ask(self._render_prompt(transcript))

    def find_with_schema_restated(self, transcript: Transcript, failure: str) -> HookCandidate:
    ...
    def _ask(self, prompt: str) -> HookCandidate:
        payload = {
            "model": self._model,
    ...
    def _parse(self, body: dict[str, Any]) -> HookCandidate:
        ...
        try:
            return HookCandidate.model_validate(parsed)
        except ValidationError as error:
            raise HookFinderError(
                f"model JSON does not match the HookCandidate contract: {error}"
            ) from error
